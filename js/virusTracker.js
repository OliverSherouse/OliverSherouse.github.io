...
viTrack.buildChart = function (canvas, title, source) {
  const chart = new Chart(canvas, {
    ...
    options: {
      ...
      onResize: (chart, size) => {
        const newdisplay = size.width > 500
...
viTrack.initRecentPerBedChart = function () {
  ...
  viTrack.initChart(
    ...
    'Johns Hopkins CSSE, CovidTracking.com, World Bank, Census, Kaiser Family Foundation',
    {
      threshold: {
        label: 'Threshold:',
        value: 0.01,
...
viTrack.initConfirmedChart = function () {
  viTrack.initChart(
    'virus-tracker-cases',
    'Confirmed Cases',
    'Johns Hopkins CSSE, CovidTracking.com',
    {
      threshold: {
        label: 'Threshold:',
        value: 100,
...
viTrack.getRecent = function () {
  const length = 21
  const recent = viTrack.byRegionOrCountry.confirmed.map(s => new viTrack.Series(s.id, s.data.map((v, i, arr) => v - (arr[i - length] || 0))))
...
viTrack.initConfirmedPerMChart = function () {
  ...
  viTrack.initChart(
    ...
    'Johns Hopkins CSSE, CovidTracking.com, World Bank, Census',
    {
      threshold: {
        label: 'Threshold:',
        value: 10,
...
viTrack.loadCSV = async function (url) {
  ...
  for (let i = 1; i < results.data.length; i++) {
    ...
    serieses.push(new viTrack.Series(`${country}----${region}`, row.slice(4).map(Number)))
  }
  return serieses
}
...
viTrack.loadRaw = async function () {
  const raw = {}
  let stateResponse = await fetch('https://covidtracking.com/api/states/daily')
  ...
  const stateObs = await stateResponse.json()
  for (let i = 0; i < stateObs.length; i++) {
    const obs = stateObs[i]
    const state = viTrack.statemap[obs.state]
    if (state === undefined) { continue }
    const id = `US----${state}`
    if (!raw.confirmed.has(id)) {
      raw.confirmed.addSeries(new viTrack.Series(id, []))
      raw.deaths.addSeries(new viTrack.Series(id, []))
    }
    raw.confirmed.get(id).data.unshift(obs.positive || 0)
    raw.deaths.get(id).data.unshift(obs.death || 0)
  }
